...
def main():
    ...
    p.add_argument('--beta', type=float, default=1.0,
                help='Weighting of kld loss term in VAE.')
    p.add_argument('--DEV', type=int, default=0,
                help='cuda device number')
    p.add_argument('--delta', type=float, default=0.001,
                help='minimal early stopping improvement')
    p.add_argument('--patience', type=int, default=5,
                help='early stopping steps to tolerate')
    opt = p.parse_args()
    ...
    model = VAE(input_dim=DATASET_INPUT_DIM[opt.dataset], hidden_dim=hidden_dim, latent_dim=opt.latent_dim, beta=opt.beta).to(device)
    ...
    early_stopping = EarlyStopping(patience=opt.patience, delta=opt.delta, path=f'{run_name}/checkpoint.pth',  verbose=True)
    prev_updates = 0
    for epoch in range(num_epochs):
        print(f'Epoch {epoch+1}/{num_epochs}')
        prev_updates = train(model, train_loader, optimizer, prev_updates, device, writer=writer)
        
        valid_loss = test(model, test_loader, prev_updates, device, opt, writer=writer, out_dim=int(np.sqrt(DATASET_INPUT_DIM[opt.dataset])))
        ...
        if early_stopping.early_stop:
            ...
            break
        visualize_latents(model, test_loader, prev_updates, device, opt, run_name, writer=writer, save=False)

    # save latent embeddings
    visualize_latents(model, test_loader, prev_updates, device, opt, run_name, writer=writer, save=True)
...
def test(model, dataloader, cur_step, device, opt, writer=None, out_dim=28):
    ...
    if writer is not None:
        ...
        # Log reconstructions
        writer.add_images('Test/Reconstructions', output.x_recon.view(-1, 1, out_dim, out_dim), global_step=cur_step)
        writer.add_images('Test/Originals', data.view(-1, 1, out_dim, out_dim), global_step=cur_step)
        
        # Log random samples from the latent space
        z = torch.randn(16, opt.latent_dim).to(device)
        samples = model.decode(z)
        writer.add_images('Test/Samples', samples.view(-1, 1, out_dim, out_dim), global_step=cur_step)
    return test_loss


def visualize_latents(model, dataloader, cur_step, device, opt, run_name, writer=None, save=False):
    """
    Tests the model on the given data.
    ...
    if save:
        ...
        df.to_pickle(join(run_name, f'vae_embeddings_latent_dim_{str(opt.latent_dim)}.pkl'))
    ...
    for label, color in zip(u_labels, colors):
        plt.scatter(latents[targets == label , 0] , latents[targets == label , 1] , label=label, color=color)
