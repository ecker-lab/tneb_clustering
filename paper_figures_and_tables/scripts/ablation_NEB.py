...
def compute_eval_points_ablation(dataset_name, desired_eval_points, cache_path):
    ...
    all_adjacencies = dict()
    # all_paths = dict()
    adjacencies, _, paths = corc.graph_metrics.tmm_gmm_neb.compute_neb_paths_batch(
        tmm.mixture_model.centers,
        tmm.mixture_model.covs,
        tmm.mixture_model.weights,
        tmm.mixture_model.df,
        iterations=500,
        num_NEB_points=100,
        num_eval_points_list=desired_eval_points,
    )

    distances = list()
    for path in paths:
        dists = dict()
        for num_eval_points in desired_eval_points:
            interpolated_path = (
                corc.graph_metrics.tmm_gmm_neb.interpolate_paths_batched(
                    path, num_eval_points
                )
            )
            logprobs = corc.graph_metrics.tmm_gmm_neb.tmm_jax(
                interpolated_path,
                tmm.mixture_model.centers,
                tmm.mixture_model.covs,
                tmm.mixture_model.weights,
...
def plot_adjs_per_dataset(adjs_per_dataset, ablation_type, datasets=None):
    ...
    for dataset in datasets:
        ...
        ax.plot(values_list, normalized_diff_list, label=dataset, marker="o")

    # Set the title and labels
    if ablation_type == "iterations":
        ax.set_title("Normalized Difference between Adjacency Matrices")
        ax.set_xlabel("Number of Iterations")
    elif ablation_type == "pathlength":
        ax.set_title("Normalized Difference between Adjacency Matrices")
        ax.set_xlabel("Number of NEB Points")
    ax.set_ylabel("Normalized Difference")
    ax.set_ylim([0, 1.1])  # Set the y-axis limit to 1.1 to make the plot more readable
    ax.legend()
