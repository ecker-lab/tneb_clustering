...
class GWGMara(Graph):
    def __init__(self, latent_dim, data=None, labels=None, path=None, n_components=10, n_neighbors=3, seed=42):
        """
    ...
    def create_graph(self, save=True, plot=True, return_graph=False):
        ...
        self.graph_data = {'nodes':cluster_means, 'edges':edges, 'nodes_org_space':gmm_fit.means_, 'norm':[np.array(list(edges.values())).min(), np.array(list(edges.values())).max()]}
    ...
    def get_dip_dict(self, means, labels, knn_dict):
        dip_dict = {}
        for cm, neighs in knn_dict.items():
            ...
            for n in list(neighs):
                ...
                dip = diptest.dipstat(np.concatenate([cluster1_proj, cluster2_proj])) * 2
                dip_list.append(dip)
            dip_dict[cm] = dip_list
        return dip_dict


    def _get_edges_dict(self, knn_dict, dip_dict):
        edges = {}

        for (cm, neighs), (_, dips) in zip(knn_dict.items(), dip_dict.items()):
    ...
    def _plt_graph_compare(self, embeddings, pred_labels, save=None):
        ...
        palette = sns.color_palette(cc.glasbey, n_colors=self.n_components) if self.n_components > 10 else sns.color_palette("tab20")
        ...
        norm = mpl.colors.Normalize(vmin=self.graph_data['norm'][0], vmax=self.graph_data['norm'][1])
