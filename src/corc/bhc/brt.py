...
class BayesianRoseTrees(api.AbstractBayesianBasedHierarchicalClustering):
    ...
    def build(self):
        ...
        while active_nodes.size > 1:
            ...
            for node in active_nodes:
                ...
                node_ch = pch[node]
                # compute log(f_k)
                node_data = self.data[np.argwhere(assignments == node).flatten()]
                ...
                lh_join = self.__compute_join(log_f_k, sum_log_p_new_node)

                lh_ab_left = self.__compute_absorb_left(
                    node_ch, log_p, new, log_f_k, sum_log_p_new_node
                )

                lh_ab_right = self.__compute_absorb_right(
                    node, log_p, new_ch, log_f_k, sum_log_p_new_node
                )

                lh_collapse = self.__compute_collapse(
                    node_ch, log_p, new_ch, log_f_k, sum_log_p_new_node
                )

                # store results
                merge_info = [new, node, lh_join, lh_ab_left, lh_ab_right, lh_collapse]
        ...
        return api.Result(
            BayesianRoseTrees.__create_arc_list(pch),
    ...
    def __init_pairs(self, n_objects, log_p):
        ...
        for i in range(n_objects):
            for j in range(i + 1, n_objects):
                ...
                tmp_merge = BayesianRoseTrees.__init_tmp_merge(merge_info, tmp_merge)
        return tmp_merge
    ...
    def __compute_absorb_left(self, node_ch, log_p, new, log_f_k, sum_log_p_new_node):
    ...
    def __compute_absorb_right(self, node, log_p, new_ch, log_f_k, sum_log_p_new_node):
    ...
    def __compute_collapse(self, node_ch, log_p, new_ch, log_f_k, sum_log_p_new_node):
        lh_collapse = -np.inf
    ...
    def __init_tmp_merge(merge_info, tmp_merge):
        return merge_info if tmp_merge is None else np.vstack((tmp_merge, merge_info))
