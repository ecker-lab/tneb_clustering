...
class Result(object):
    def __init__(
        self, arc_list, node_ids, last_log_p, weights, hierarchy_cut, n_clusters
    ):
    ...
    def get_dendrogram(self):
        size = len(self.arc_list) // 2 + 1
        dendrogram = np.ones((size - 1, 4), dtype=int) * -1

        for arc in self.arc_list:
            index = arc.source - size
            if dendrogram[index, 0] == -1:
                dendrogram[index, 0] = arc.target
            else:
                dendrogram[index, 1] = arc.target
        return dendrogram
