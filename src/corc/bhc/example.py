...
from bhc import BayesianHierarchicalClustering, BayesianRoseTrees, NormalInverseWishart
...
def run_bhc(data):
    ...
    bhc_result = BayesianHierarchicalClustering(
        data, model, alpha, cut_allowed=True
    ).build()
    end_time = time.time()
    print(f"Time taken for BHC: {end_time - start_time:.2f} seconds")

    with open("results/bhc.pickle", "wb") as f:
        pickle.dump(bhc_result, f)

    build_graph(bhc_result.node_ids, bhc_result.arc_list, "results/bhc_plot")
...
def run_brt(data):
    ...
    start_time = time.time()
    brt_result = BayesianRoseTrees(data, model, alpha, cut_allowed=True).build()
    print(f"Time taken for BRT: {time.time() - start_time:.2f} seconds")

    with open("results/brt.pickle", "wb") as f:
        pickle.dump(brt_result, f)

    build_graph(brt_result.node_ids, brt_result.arc_list, "results/brt_plot")
