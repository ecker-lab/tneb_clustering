...
def make_gaussians(
    ...
    if equal_sized_clusters:
        ...
        n_samples_around_c = np.random.randint(
            1, n_samples // n_centers, size=n_centers - 1
        )
        n_samples_around_c = np.concatenate(
            (n_samples_around_c, [n_samples - n_samples_around_c.sum()])
        )
    ...
    if not isinstance(std, np.ndarray):
        assert isinstance(std, list) or isinstance(
            std, float
        ), f"Std needs to be float or list or np.ndarray"
        std = np.array(std) if isinstance(std, list) else np.array([std] * n_centers)
    assert (
        len(std) == n_centers
    ), f"Number of stds needs to be same as number of cluster centers."
